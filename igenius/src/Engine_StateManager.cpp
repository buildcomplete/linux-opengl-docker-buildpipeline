...
StateContext Engine_StateManager::HandleEvents()
{
    ...
    targetState = (INPUT_STATE_FLAGS)(targetState | ((IsKeyDown(KEY_LEFT_SHIFT)) ? IG_INPUT_SELECT_MODE : IG_INPUT_ZERO));
...
{       
    ...
    if (state.DidExitState(IG_INPUT_SELECT_MODE))
    {
        std::cout << "Close, or not to close ? we probable need to select an option to close";
    }
