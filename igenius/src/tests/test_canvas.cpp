...
void test_basic_canvas_operations()
{
...

    {
        ...
        test_assert(
            ...
            "test adding component outside boundaries fails",
            "c.AddComponent(...");
        test_equals(c.GetCellInfo(0, 0).componentId, (std::uint8_t)0, "Test cell has no component");
    }
}
...
void test_moving_component_canvas_operations()
{
...

    {
        ...
        c.AddComponent(ComponentFactory::GetBluePrint(CMPNAMES::DRAWING_COMPONENT), 1, 1);
        test_assert(
            !c.MoveComponentTo(1, {-1, -1}),
            "!c.MoveComponentTo(1, {-1, -1})",
            "Should fail to move to invalid position");
        test_equals(c.GetCellInfo(1, 1).componentId, (std::uint8_t)1, "Component 1 should not be moved from 1,1");
    }
}
